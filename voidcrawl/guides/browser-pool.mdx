...
The nested `BrowserConfig` is applied to every Chrome the pool launches. One constraint: `BrowserConfig.user_data_dir` (a [persistent profile](/voidcrawl/guides/profiles/#dedicated-persistent-profiles-browserconfiguser_data_dir)) requires `browsers=1` and no `chrome_ws_urls` -- Chrome locks the profile directory, so it can't be shared across processes. The pool raises `ValueError` otherwise.
...
| `CHROME_LAUNCH_TIMEOUT_SECS` | `45` | Max seconds to wait for Chrome to report its DevTools WebSocket URL at launch. Read by the Rust core (applies to `BrowserSession` too, not just pools). Raise it on slow cold starts -- e.g. CI runners without a working GPU, where the stealth GPU flags make Chrome's GPU process crash-loop before settling. |
