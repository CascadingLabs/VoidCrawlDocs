...
### Hardware GPU (avoid SwiftShader)

By default a headless browser renders WebGL with **SwiftShader** (software) -- a bot signal that managed Turnstile and similar WAFs weigh (see [Stealth](/voidcrawl/guides/stealth/)). The headless image ships the Mesa Vulkan/GL drivers and the container Chrome launches with `--headless=new` + ANGLE + `--disable-gpu-sandbox`, so passing the host's render node gives **hardware** WebGL. `docker/docker-compose.yml` already declares it:

```yaml
devices:
  - /dev/dri:/dev/dri
group_add: ["render"]   # render-node GID; numeric on some hosts
```

Intel and AMD work through `/dev/dri` out of the box; NVIDIA needs the `nvidia-container-toolkit` (`--gpus all`) instead. With no GPU passed through the container falls back to SwiftShader -- fine for bulk scraping, but for managed-Turnstile targets prefer the headful container below.

### DNS override

If the container inherits an unreachable host stub resolver and Chrome cannot resolve public sites, pass deterministic nameservers at startup:
...
Leave the variable unset for Docker's normal resolver behavior.

### Persistent profiles

By default Chrome profiles live under `/tmp` and reset with the container. To persist cookies, logins, and Cloudflare clearance, set `CHROME_PROFILES_DIR=/profiles`; the compose files already mount a named profile volume there:

```bash
CHROME_PROFILES_DIR=/profiles docker compose -f docker/docker-compose.yml up -d
# reset saved profiles: docker compose -f docker/docker-compose.yml down -v
```

Each Chrome owns one subdirectory (`chrome-profile-1`, `chrome-profile-2`, ...). Chrome locks a profile while running, so do not share one profile dir across multiple Chrome processes.
...
Attached/headful tabs keep Chrome's native fingerprint and avoid pre-navigation stealth mutations. The container supplies the human-shaped launch surface, GPU, and profile state; VoidCrawl lazily escalates CDP instrumentation only when a feature such as network capture needs it.
