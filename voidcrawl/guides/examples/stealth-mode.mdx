...
The key difference is `navigator.webdriver`. With stealth on it's a native **`false`** -- VoidCrawl does **not** patch it in JS (deleting it to `undefined` is itself a tell); the launch flag `--disable-blink-features=AutomationControlled` produces a real `false`. With stealth off, Chrome sets it to `true`, which is the primary signal WAFs check.
