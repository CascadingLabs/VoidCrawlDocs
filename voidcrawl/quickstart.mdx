...
async def main():
    async with BrowserSession(BrowserConfig()) as session:
        async with session.page("https://qscrape.dev") as page:
...
- `goto(url, wait_until="networkidle", timeout=30.0)` navigates and waits for strict network idle. Navigation timeouts raise `NavigationTimeoutError` with structured phase and timing details.
- `navigate(url)` dispatches navigation without waiting for network idle. Use it when a selector or expected API response is the meaningful readiness condition. Reading `content()` immediately after `navigate()` may return an empty or partial page.
- `BrowserSession.page()` is the safest low-level tab API. Its async context manager closes the tab on success, error, or cancellation.

## Next Steps

- [Browser Pool](/voidcrawl/guides/browser-pool/) -- understand cold start vs tab reuse
- [Page Lifecycle and Response Capture](/voidcrawl/guides/page-lifecycle-and-response-capture/) -- manage tabs and capture bounded API payloads
